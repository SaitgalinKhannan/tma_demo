...
import {Box, Dialog, DialogContent, DialogTitle, Paper, Typography, Button, DialogActions} from "@mui/material";
import NavigationBar from "./components/NavigationBar.tsx";
import {isPhotoMessage, isTextMessage, Message, MessageStatus, TextMessage} from "./model/Message.ts";
...
const generateMessages = (): TextMessage[] => {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        messages.push({
            ...
            receiverStatus: i < 15 ? MessageStatus.READ : (i < 18 ? MessageStatus.DELIVERED : MessageStatus.SENT),
            type: 'TextMessage',
    ...
    return (
        <>
            ...
            >
                ...
                >
                    {messages.map((message) => (
                        <div
                            ...
                            style={{
                                display: 'flex',
                                justifyContent: message.sender === id.toString() ? 'flex-end' : 'flex-start',
                            }}
                        >
                            <Paper
                                elevation={2}
                                sx={{
                                    ...
                                    padding: "10px",
                                    maxWidth: '70%',
                                    backgroundColor: message.sender === id?.toString() ? '#e3f2fd' : '#f5f5f5',
                                }}
                            >
                                {
...

                                {
                                    isPhotoMessage(message) && <img
                                        ...
                                        onClick={() => handleClickToImage(getImage(message.photoUrl))}
                                    />
                                }
                                <Typography variant="caption"
                                            sx={{display: 'block', mt: 1, textAlign: 'right', fontSmooth: "auto"}}>
                ...
                >
                    <div className="input-container">
                        <button className="chat-attach-button" onClick={() => {
                    ...
                    <button ref={buttonRef} className="send-button" /*style={{bottom: `${buttonBottom}px`}}*/
            ...
            <CommandBar commands={commands} showCommands={showCommands} setShowCommands={setShowCommands}
                        sendMessage={handleSendTextMessage}/>

            {
                previewUrl && <Dialog
                    open={showPhotoSendDialog}
                    onClose={handleFileSendDialogClose}
                >
